function [delta, maxloglik, fitted, iter, phat, sdhat] = spglarmascore(Y, X, phi_lags, theta_lags,link,tests)
...
conn = @(para)constraintsglarmascore(para, Y, X, phi_lags, theta_lags,link) ; %passing data to constraints
...
options = optimoptions('fmincon','MaxFunEvals',1e5, 'MaxIter', 1e5, 'TolFun', 1e-8, ...
    ...
    'Display', 'off', 'SpecifyConstraintGradient', false, 'SpecifyObjectiveGradient', true,...
    'UseParallel',true);
...
%options = optimset('MaxFunEvals',1e5, 'MaxIter', 1e5, 'TolFun', 1e-8, 'TolCon', 1e-8, 'TolX', 1e-8, 'Algorithm', 'interior-point', 'Display', 'off', 'GradConstr', 'off', 'GradObj', 'on','UseParallel','always');
%[param, fvalue, exitflag, output, ~, ~, ~]=fmincon(ell, para0, [], [], [], [], [], [], conn, options) ;
